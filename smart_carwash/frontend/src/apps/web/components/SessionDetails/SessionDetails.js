...
const ChemistryEnableButton = ({ session, theme, onChemistryEnabled }) => {
  ...
  const handleEnableChemistry = async () => {
    ...
    try {
      setIsEnabling(true);
      await WebApiService.enableChemistry(session.id);
      
      // Немедленно обновляем данные сессии для мгновенного отображения изменений
      try {
        const updatedSessionData = await WebApiService.getSessionById(session.id);
 ...
 */
const SessionDetails = ({ theme = 'light', user, basePath: basePathProp }) => {
  ...
  const loadSessionPayments = async () => {
    ...
    try {
      ...
      const payments = await WebApiService.getSessionPayments(sessionId);
  ...
  const startSessionPolling = () => {
    ...
    pollingInterval.current = setInterval(async () => {
      try {
        const sessionData = await WebApiService.getSessionById(sessionId);
        ...
        if (sessionData && sessionData.session) {
          ...
          try {
            const paymentResponse = await WebApiService.getUserSessionForPayment(sessionData.session.user_id);
  ...
  const handleExtendSession = async () => {
    ...
    try {
      ...
      const response = await WebApiService.extendSessionWithPayment(sessionId, selectedExtensionTime, selectedChemistryTime);
  ...
  const handleBuyChemistry = async () => {
    ...
    try {
      ...
      const response = await WebApiService.extendSessionWithPayment(sessionId, 0, selectedChemistryTime);
  ...
  const handleCompleteSession = async () => {
    ...
    try {
      ...
      const response = await WebApiService.completeSession(sessionId);
      
      if (response && response.session) {
        // Немедленно обновляем данные сессии для мгновенного отображения изменений
        try {
          const updatedSessionData = await WebApiService.getSessionById(sessionId);
          if (updatedSessionData && updatedSessionData.session) {
            ...
            // Иначе, если у сессии есть назначенный бокс, получаем информацию о нем
              ...
              const queueStatus = await WebApiService.getQueueStatus();
  ...
  const fetchSessionDetails = async () => {
    try {
      setLoading(true);
      const response = await WebApiService.getSessionById(sessionId);
      ...
      if (response && response.session) {
        ...
        try {
          const paymentResponse = await WebApiService.getUserSessionForPayment(response.session.user_id);
          if (paymentResponse && paymentResponse.payment) {
            setPayment(paymentResponse.payment);
          }
        } catch (paymentError) {
  ...
  const handleStartSession = async () => {
    try {
      ...
      const response = await WebApiService.startSession(sessionId);
  ...
  const handleCancelSession = async () => {
    ...
    try {
      setIsCanceling(true);
      const response = await WebApiService.cancelSession(session.id, user.id);
                ...
                <Button 
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const response = await WebApiService.getUserSessionForPayment(session.user_id);
