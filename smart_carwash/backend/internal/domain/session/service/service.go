...
func (s *ServiceImpl) CreateSessionWithPayment(ctx context.Context, req *models.CreateSessionWithPaymentRequest) (*models.CreateSessionWithPaymentResponse, error) {
	...
	// LOYALTY: Проверка с защитой от race condition
	if req.ServiceType == "wash" && req.WithChemistry && s.userService != nil {
		var shouldCreateFree bool
		err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			...
			if user.IsSubscribedToChannel && user.CompletedWashesCount > 0 &&
				user.CompletedWashesCount%10 == 0 {
				shouldCreateFree = true
			}
			return nil
		})
		if err == nil && shouldCreateFree {
...
func (s *ServiceImpl) CompleteSession(ctx context.Context, req *models.CompleteSessionRequest) (*models.CompleteSessionResponse, error) {
	...
	if s.washboxService == nil {
		...
		// LOYALTY: Инкремент (только платные!)
		if session.ServiceType == "wash" && session.WasChemistryOn && !session.LoyaltyCounted && s.userService != nil && s.paymentService != nil {
			payment, err := s.paymentService.GetMainPaymentBySessionID(ctx, session.ID)
			if err != nil {
				logger.Printf("LOYALTY WARNING: Платеж не найден session_id=%s", session.ID)
			} else if payment != nil && payment.Amount > 0 {
...
func (s *ServiceImpl) CompleteSessionWithoutRefund(ctx context.Context, sessionID uuid.UUID, completionSource string) error {
	...
	if s.washboxService == nil {
		...
		if session.ServiceType == "wash" && session.WasChemistryOn && !session.LoyaltyCounted && s.userService != nil && s.paymentService != nil {
			payment, err := s.paymentService.GetMainPaymentBySessionID(ctx, session.ID)
			if err == nil && payment != nil && payment.Amount > 0 {
				user, _ := s.userService.GetUserByID(ctx, session.UserID)
				if user != nil && user.IsSubscribedToChannel {
					s.userService.IncrementWashCount(ctx, session.UserID)
					session.LoyaltyCounted = true
...
func (s *ServiceImpl) createFreeSession(ctx context.Context, req *models.CreateSessionWithPaymentRequest) (*models.CreateSessionWithPaymentResponse, error) {
	...
	session.StatusUpdatedAt = time.Now()
	s.repo.UpdateSession(ctx, session)
	return &models.CreateSessionWithPaymentResponse{Session: *session, Payment: nil}, nil
}
